 ...
 * @author Savindi
 */
public class DamageProductList extends javax.swing.JFrame {

    /**
     * Creates new form UserManagement
     */
    public DamageProductList(String username) {
        initComponents();
    ...
    private void loadTable() {
        try {
            ResultSet resultSet = MySQL.execute("SELECT * FROM `damaged`\n"
            ...
            while (resultSet.next()) {
                ...
                vector.add(resultSet.getString("damaged.stock_id"));
                vector.add(resultSet.getString("product.name"));
                vector.add(resultSet.getString("damaged.reason"));
                vector.add(resultSet.getString("stock.exp"));
                vector.add(resultSet.getString("damaged.date_time"));
                vector.add(resultSet.getString("damaged.user_id"));
                vector.add(resultSet.getString("damaged.qty"));
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Damaged Products");
        ...
        jComboBox7.setFont(new java.awt.Font("Quicksand Medium", 0, 14)); // NOI18N
        jComboBox7.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Select", "Damaged", "Expired", "Other" }));
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Stock Id", "Product Name", "Reason", "EXP", "Added Date", "Added Employee", "Qty", "Buying Price"
            }
        ) {
        ...
        if (search.equals("")) {
            ...
            try {
                ResultSet resultSet = MySQL.execute("SELECT * FROM `damaged`\n"
                        + "INNER JOIN `stock` ON `stock`.`id` = `damaged`.`stock_id`\n"
                        + "INNER JOIN `product` ON `stock`.`product_id` = `product`.`id`\n"
                        + "INNER JOIN `grn_item` ON `grn_item`.`stock_id` = `stock`.`id` WHERE `damaged`.`stock_id` = '" + search + "' ");
    ...
    private void jComboBox7ItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_jComboBox7ItemStateChanged
        String reason = String.valueOf(jComboBox7.getSelectedItem());
        ...
        if (reason.equals("Select")) {
            ...
            try {
                ResultSet resultSet = MySQL.execute("SELECT * FROM `damaged`\n"
                        + "INNER JOIN `stock` ON `stock`.`id` = `damaged`.`stock_id`\n"
                        + "INNER JOIN `product` ON `stock`.`product_id` = `product`.`id`\n"
                        + "INNER JOIN `grn_item` ON `grn_item`.`stock_id` = `stock`.`id` WHERE `damaged`.`reason` LIKE '%" + reason + "%' ");
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        try {
            ...
            if (dFrom == null & dTo == null) {
                resultSet = MySQL.execute("SELECT * FROM `damaged`\n"
                        + "INNER JOIN `stock` ON `stock`.`id` = `damaged`.`stock_id`\n"
                        + "INNER JOIN `product` ON `stock`.`product_id` = `product`.`id`\n"
                        + "INNER JOIN `grn_item` ON `grn_item`.`stock_id` = `stock`.`id`");
                ...
                resultSet = MySQL.execute("SELECT * FROM `damaged`\n"
                        + "INNER JOIN `stock` ON `stock`.`id` = `damaged`.`stock_id`\n"
                        + "INNER JOIN `product` ON `stock`.`product_id` = `product`.`id`\n"
                        + "INNER JOIN `grn_item` ON `grn_item`.`stock_id` = `stock`.`id` WHERE DATE(`date_time`) >= '" + dateFrom + "'");
            } else if (dFrom == null & dTo != null) {
                resultSet = MySQL.execute("SELECT * FROM `damaged`\n"
                        + "INNER JOIN `stock` ON `stock`.`id` = `damaged`.`stock_id`\n"
                        + "INNER JOIN `product` ON `stock`.`product_id` = `product`.`id`\n"
                        + "INNER JOIN `grn_item` ON `grn_item`.`stock_id` = `stock`.`id` WHERE DATE(`date_time`) <= '" + dateTo + "'");
            } else if (dFrom != null & dTo != null) {
                resultSet = MySQL.execute("SELECT * FROM `damaged`\n"
                        + "INNER JOIN `stock` ON `stock`.`id` = `damaged`.`stock_id`\n"
                        + "INNER JOIN `product` ON `stock`.`product_id` = `product`.`id`\n"
                        + "INNER JOIN `grn_item` ON `grn_item`.`stock_id` = `stock`.`id` WHERE DATE(`date_time`) >= '" + dateFrom + "' AND DATE(`date_time`) <= '" + dateTo + "'");
            }
